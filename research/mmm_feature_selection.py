...
class MMMFeatureSelection(object):
    def _results(self, df: pd.DataFrame):
        ...
        results_final = results.loc[results.groupby('type')['avg_rmse'].idxmin()]
        return results_final

    def rf(self, df: pd.DataFrame, dv_col: str, transformed_cols: [], cv: int = 10) -> dict:
        """
        Fits a random forest model using 5-fold cv to obtain an average performance score for each transformed feature.
        For each media channel, the feature with the best average performance is used for the final model fit.
        """
        df_fs = df.copy()
        ...
        for i in transformed_cols:
            ...
            avg_score = pd.DataFrame(
                {'feature': [i], 'type': [i.split('adstock')[0].replace('_', '')], 'avg_rmse': [np.mean(scores * -1)]})
    ...
    def best_feat_distribution(self, df: pd.DataFrame, transformed_cols: [], n: int = 100, cv: int = 10):
        ...
        df_out = df_out.groupby(['type', 'feature']).size().reset_index(name='counts')
        return df_out
