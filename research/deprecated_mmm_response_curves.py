...
class MMMResponseCurves(object):
    ...
    def _overall_response_aggr(self, df: pd.DataFrame, feature: str, max_freq: int,
                               increment: int) -> []:
        """
        """
        # rf response curve
    ...
    def responses(self, curve_type: str, model, x: pd.DataFrame, feature: str, max_freq: int,
                  increment: int,
        ...
        for i in range(0, max_freq + 1, increment):
            ...
            if 'adstock' in feature:
                df_sim[feature] = transform.segment_adstock(df_sim, feature, segment,
                                                            float(feature.split('_')[-1]))
            df_preds[f"{feature}_preds_{i}"] = model.predict(
                df_sim.drop([segment, date], errors='ignore', axis=1))
        # aggregate predictions
        if curve_type == 'segment':
            df_aggr = df_preds.drop([date], axis=1).groupby(segment).mean().reset_index()
            resp_final, popt, fig_hill, fig_raw, scaler_x, scaler_y = self._segment_response_aggr(
                df_aggr, feature,
                max_freq, increment,
                segment)
        elif curve_type == 'overall':
            # df_aggr = df_preds.drop([segment], axis=1).groupby(date).sum().reset_index()
            resp_final, popt, fig_hill, fig_raw, scaler_x, scaler_y = self._overall_response_aggr(
                df_preds, feature,
                max_freq, increment)
        return {'resp_df': resp_final, 'optimal_hill': popt, 'fig_hill': fig_hill,
                'fig_raw': fig_raw, 'scaler_x': scaler_x, 'scaler_y': scaler_y}
    ...
    def responses_segment(self, model, x: pd.DataFrame, feature: str, max_freq: int, increment: int,
        ...
        # final out
        resp_dict = {'resp_df': resp_final, 'optimal_hill': opt_hill, 'fig_hill': fig_hill,
                     'fig_raw': fig_raw}
        return resp_dict
